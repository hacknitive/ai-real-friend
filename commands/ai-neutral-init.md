---
description: Install ai-neutral hooks + skill into one or more Claude Code config dirs (mirrors caveman-init)
argument-hint: "[--dry-run] [--config-dir PATH] [--all-accounts] [--no-statusline] [--uninstall]"
---

Run the ai-neutral installer against the requested Claude Code config dir(s). By default installs into `$CLAUDE_CONFIG_DIR` (falls back to `~/.claude`). Pass `--all-accounts` to install into every `~/.claude-*` directory that looks like a config dir (multi-account setups).
...
2. Otherwise clone the repo and run the installer:
   `git clone https://github.com/hacknitive/ai-neutral /tmp/ai-neutral && node /tmp/ai-neutral/bin/install.js $ARGUMENTS`
